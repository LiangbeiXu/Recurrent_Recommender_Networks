...
def main():
    ...
    model = RRN2(data_name='Assistment09', item='problem', epochs = 5)
...
class RRN2:
    ...
    def __init__(self, data_name='Assistment09', item='problem', epochs = 5, file_path = "/home/lxu/Documents/StudentLearningProcess/2015_ASSISTment.pickle" ):
        # params parser
        self.batch_size = 200
        self.n_step = 1
        self.lr = 0.0001
    ...
    def calculate_embedding(self):
        ...
        # PlotLoss(model, name)
        print('global bias, user bias, skill bias', fm.beta_global, fm.beta_user, fm.beta_prob)
        self.global_bias_constant = fm.beta_global
    ...
    def add_placeholder(self):
        ...
        # self.global_bias = tf.placeholder(tf.float32, shape=[None, 1], name="global_bias")
    ...
    def add_embedding_layer(self):
        with tf.name_scope("userID_embedding"):
            ...
            self.uid_layer = tf.reshape(uid_layer, [-1, 128])
    ...
    def add_rnn_layer(self):
        with tf.variable_scope("user_rnn_cell"):
            ...
            userOutputs, userStates = tf.nn.dynamic_rnn(userCell, cellInput, dtype=tf.float32)
    ...
    def add_pred_layer(self):
        W = {
            'userOutput': tf.Variable(tf.random_normal(shape=[128, 128], stddev=0.1)),
            'movieOutput': tf.Variable(tf.random_normal(shape=[128, 128], stddev=0.1))
        }
        b = {
    ...
    def createFeedDict(self, data, dropout=1.):
        ...
        for i in data.values:
            userID.append([i[0]])
            userID2.append(self.user_embedding_constant[np.int(i[0]),:])
            movieID.append(self.item_embedding_constant[np.int(i[1]),:])
            ratings.append([float(i[2])])
            # global_bias.append([copy.deepcopy(self.global_bias_constant)])
        ...
        return {
            ...
            self.userID2:np.array(userID2),
            self.movieID: np.array(movieID),
            self.rating: np.array(ratings),
            # self.global_bias: np.array(global_bias),
